 ...
 * --------------------------------------------------------------------------- */
add_action( 'admin_enqueue_scripts', 'consultancy_jq_migration', 0 );
add_action( 'wp_enqueue_scripts', 'consultancy_jq_migration', 150 );
function consultancy_jq_migration() {
	wp_enqueue_script('jquery-migrate');
}

add_action('wp_enqueue_scripts', 'consultancy_enqueue_scripts');
if(!function_exists('consultancy_enqueue_scripts')) {
function consultancy_enqueue_scripts() {
	...
	// scipts variable -----------------------------------------------------------
	$loadingbar = (int) get_theme_mod( 'use-site-loader', consultancy_defaults('use-site-loader') );
	$loadingbar = !empty( $loadingbar ) ?  "enable" : "disable";
	
	$stickynav = (int) get_theme_mod( 'enable-sticy-nav', consultancy_defaults('enable-sticy-nav') );
	$stickynav = !empty( $stickynav ) ?  "enable" : "disable";

	if(is_rtl()) $rtl = true; else $rtl = false;

	$htype = get_theme_mod( 'header-type', consultancy_defaults('header-type') );
	$stickyele = "";
	switch( $htype ){
	...
	if( is_page_template( 'tpl-header2.php' ) )
		$stickyele = ".main-header-wrapper";
	...
	if( cs_get_option('enable-cookie-consent') == "true" ) {
		wp_enqueue_script('consultancy-cookie-js', get_theme_file_uri('/framework/js/cookieconsent.js'), array(), false, true);
	}

	wp_enqueue_script('consultancy-popup-js', get_theme_file_uri('/framework/js/magnific/jquery.magnific-popup.min.js'), array(), false, true);
	...
	wp_enqueue_script('consultancy-jqcustom', get_theme_file_uri('/framework/js/custom.js'), array(), false, true);

	wp_localize_script('consultancy-jqcustom', 'dttheme_urls', array(
		'theme_base_url' => esc_js(CONSULTANCY_THEME_URI),
		'framework_base_url' => esc_js(CONSULTANCY_THEME_URI).'/framework/',
		...
		'advOptions' => esc_html__('Show Advanced Options', 'consultancy'),
	));
	...
	if( isset($picker) ) {
		...
		wp_enqueue_script('consultancy-jqcpanel', get_theme_file_uri('/framework/js/controlpanel.js'),array(),false,true);
	}
}
}
...
* --------------------------------------------------------------------------- */
if(!function_exists('consultancy_scripts_custom')) {
function consultancy_scripts_custom() {
	
	$enable_custom_js = (int) get_theme_mod( 'enable-custom-js', consultancy_defaults('enable-custom-js') );
	$custom_js = get_theme_mod( 'custom-js', '');
	
	if( !empty( $enable_custom_js ) && !empty( $custom_js ) ){
		wp_add_inline_script('consultancy-jqcustom', consultancy_wp_kses(stripslashes($custom_js)) ,'after');
	}
}
}
add_action('wp_enqueue_scripts', 'consultancy_scripts_custom', 100);

/* ---------------------------------------------------------------------------
 * Loading Theme Styles
 * --------------------------------------------------------------------------- */
add_action( 'wp_enqueue_scripts', 'consultancy_enqueue_styles', 101 );
if(!function_exists('onsultancy_enqueue_styles')) {
function consultancy_enqueue_styles() {

	// site icons ---------------------------------------------------------------
	if ( ! has_site_icon() ):
		$url = CONSULTANCY_THEME_URI . "/images/favicon.ico";
		echo "<link href='$url' rel='shortcut icon' type='image/x-icon' />\n";		
	endif;

	// wp_enqueue_style ---------------------------------------------------------------
	wp_enqueue_style( 'consultancy', get_stylesheet_uri(), false, CONSULTANCY_THEME_VERSION, 'all' );

	wp_enqueue_style( 'consultancy-base',		  get_theme_file_uri('/css/base.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-grid', 		  get_theme_file_uri('/css/grid.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-widget', 	  get_theme_file_uri('/css/widget.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-layout', 	  get_theme_file_uri('/css/layout.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-blog',	      get_theme_file_uri('/css/blog.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-portfolio',	  get_theme_file_uri('/css/portfolio.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-contact',	  get_theme_file_uri('/css/contact.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	wp_enqueue_style( 'consultancy-custom-class', get_theme_file_uri('/css/custom-class.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	wp_enqueue_style( 'prettyphoto',	get_theme_file_uri('/css/prettyPhoto.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	...
	if( cs_get_option( 'enable-comingsoon' ) )
		wp_enqueue_style("consultancy-comingsoon",  get_theme_file_uri("/css/comingsoon.css"), false, CONSULTANCY_THEME_VERSION, 'all' );

	// notfound css
	if ( is_404() )
		wp_enqueue_style("consultancy-notfound",	get_theme_file_uri("/css/notfound.css"), false, CONSULTANCY_THEME_VERSION, 'all' );

	// loader css
	$loadingbar = (int) get_theme_mod( 'use-site-loader', consultancy_defaults('use-site-loader') );
	if( !empty( $loadingbar ) )
		wp_enqueue_style("consultancy-loader", 		get_theme_file_uri("/css/loaders.css"), false, CONSULTANCY_THEME_VERSION, 'all' );

	// woocommerce css
	if( function_exists( 'is_woocommerce' ) )
		wp_enqueue_style( 'consultancy-woo', 		get_theme_file_uri('/css/woocommerce.css'), 'woocommerce-general-css', CONSULTANCY_THEME_VERSION, 'all' );

	// skin css
	$use_predefined_skin = (int) get_theme_mod( 'use-predefined-skin', consultancy_defaults('use-predefined-skin') );
	if( !empty( $use_predefined_skin ) ) :
		$skin = get_theme_mod( 'predefined-skin', consultancy_defaults('predefined-skin') );
		wp_enqueue_style("consultancy-skin", 	get_theme_file_uri("/css/skins/$skin/style.css"));
	endif;

	// tribe-events -------------------------------------------------------------------
	wp_enqueue_style( 'consultancy-customevent', 		get_theme_file_uri('/tribe-events/custom.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	// responsive ---------------------------------------------------------------------
	$responsive = (int) get_theme_mod( 'site-responsive', consultancy_defaults('site-responsive') );
	if( !empty( $responsive ) )
		wp_enqueue_style( "consultancy-responsive",  	get_theme_file_uri('/css/responsive.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
		
		// cookie-consent -----------------------------------------------------------------
	if( cs_get_option('enable-cookie-consent') == "true" ) {
		wp_enqueue_style( 'consultancy-cookie-css', 		get_theme_file_uri('/css/cookieconsent.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
	}

	wp_enqueue_style( 'consultancy-popup-css', 		get_theme_file_uri('/framework/js/magnific/magnific-popup.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	// gutenberg css ---------------------------------------------------------------------
	wp_enqueue_style( 'consultancy-gutenberg', get_theme_file_uri('/css/gutenberg.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	// custom css ---------------------------------------------------------------------
	wp_enqueue_style( 'consultancy-custom', 			get_theme_file_uri('/css/custom.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	// jquery scripts --------------------------------------------
	wp_enqueue_script('modernizr-custom', 	get_theme_file_uri('/framework/js/modernizr.custom.js'), array('jquery'));

	// rtl ----------------------------------------------------------------------------
	if(is_rtl()) wp_enqueue_style('consultancy-rtl', 	get_theme_file_uri('/css/rtl.css'), false, CONSULTANCY_THEME_VERSION, 'all' );

	# Sub Menu Wrapper Shadow
	$enable_sub_menu_shadow = (int) get_theme_mod( 'allow-sub-menu-box-shadow', consultancy_defaults('allow-sub-menu-box-shadow') );
	if( !empty( $enable_sub_menu_shadow ) ) {

		$h_shadow = (int) get_theme_mod( 'sub-menu-box-h-shadow', consultancy_defaults('sub-menu-box-h-shadow') );
		$h_shadow .= 'px ';

		$v_shadow = (int) get_theme_mod( 'sub-menu-box-v-shadow', consultancy_defaults('sub-menu-box-v-shadow') );
		$v_shadow .= 'px ';

		$blur_shadow = (int) get_theme_mod( 'sub-menu-box-blur-shadow', consultancy_defaults('sub-menu-box-blur-shadow') );
		$blur_shadow .= 'px ';

		$spread_shadow = (int) get_theme_mod( 'sub-menu-box-spread-shadow', consultancy_defaults('sub-menu-box-spread-shadow') );
		$spread_shadow .= 'px ';

		$color_shadow = get_theme_mod( 'sub-menu-box-shadow-color', '' );

		$inset_shadow = (int) get_theme_mod( 'sub-menu-box-shadow-inset', consultancy_defaults('sub-menu-box-shadow-inset') );
		...
		wp_add_inline_style( 'consultancy', $css );		
	}

	# Sub Menu Wrapper Gradient BG
	$enable_sub_menu_bg = (int) get_theme_mod( 'allow-sub-menu-bg-color', consultancy_defaults('allow-sub-menu-bg-color') );
	$sub_menu_bg_type = get_theme_mod( 'sub-menu-bg-color-type', consultancy_defaults('sub-menu-bg-color-type') );
	...
	if( empty( $use_predefined_skin ) ) {
		...
		if( !empty( $primary_color ) ) {
			...
			$rgba = consultancy_hex2rgb( $primary_color );
			$rgba = implode(',', $rgba);

			# Header
			$header_type = get_theme_mod('header-type',consultancy_defaults('header-type'));
			...
			if( function_exists( 'is_woocommerce' ) ){
				...
				$css .= '.woo-type15 ul.products li.product .product-thumb a.add_to_cart_button:after, .woo-type15 ul.products li.product .product-thumb a.button.product_type_simple:after, .woo-type15 ul.products li.product .product-thumb a.button.product_type_variable:after, .woo-type15 ul.products li.product .product-thumb a.added_to_cart.wc-forward:after, .woo-type15 ul.products li.product .product-thumb a.add_to_wishlist:after, .woo-type15 ul.products li.product .product-thumb .yith-wcwl-wishlistaddedbrowse a:after, .woo-type15 ul.products li.product .product-thumb .yith-wcwl-wishlistexistsbrowse a:after, .dt-sc-button.filled:hover {
				...
				$css .= '.dt-sc-icon-box.type5:hover .icon-wrapper {
		...
		if( !empty( $secondary_color ) ) {
			...
			$rgba = consultancy_hex2rgb( $secondary_color );
			$rgba = implode(',', $rgba);
		...
		if( !empty( $tertiary_color ) ) {
			...
			$rgba = consultancy_hex2rgb( $tertiary_color );
			$rgba = implode(',', $rgba);
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_ssl')) {
function consultancy_ssl( $echo = false ){
	$ssl = '';
	if( is_ssl() ) $ssl = 's';
	if( $echo ){
		echo "{$ssl}";
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_body_classes')) {
function consultancy_body_classes( $classes ) {

	// layout
	$classes[] 		= 	'layout-'. get_theme_mod( 'site-layout', consultancy_defaults('site-layout') );
	...
	$htrans 		= 	get_theme_mod( 'header-transparency', consultancy_defaults('header-transparency') );
	if(isset($htrans)):
		$classes[]	=	get_theme_mod( 'header-transparency', consultancy_defaults('header-transparency') );
	endif;	

	$stickyhead = (int) get_theme_mod( 'enable-sticy-nav', consultancy_defaults('enable-sticy-nav') );
	if( !empty( $stickyhead ) ) {
		$classes[]	=	'sticky-header';
	}

	$standard		=	get_theme_mod( 'header-position', consultancy_defaults('header-position') );
