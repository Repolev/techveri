...
if ( ! class_exists( 'Gutenberg_Editor_CSS' ) ) :
	...
	class Gutenberg_Editor_CSS {
		...
		function __construct() {
			...
			add_action( 'current_screen', array( $this,  'consultancy_current_screen_hook' ) );

			if ( class_exists( 'Classic_Editor' ) ) {
				add_filter( 'tiny_mce_before_init', array( $this, 'consultancy_editor_dynamic_styles' ) );
			}
			
			add_action( 'enqueue_block_editor_assets', array( $this, 'consultancy_backend_editor_styles' ) );

		}

		public function consultancy_current_screen_hook( $current_screen ) {

			if ( 'post' == $current_screen->base ) {

				$urls = CONSULTANCY_Kirki::enqueue_fonts_url();
		...
		public function consultancy_custom_fonts() {
		...
		public function consultancy_editor_dynamic_styles( $mceInit ) {
	
			$styles = '';

			$styles .= $this->consultancy_custom_fonts();
			$styles .= $this->consultancy_backend_classic_editor_styles();
		...
		public function consultancy_backend_classic_editor_styles() {
			...
			$status_of_body_content_typo = get_theme_mod( 'customize-body-content-typo', consultancy_defaults('customize-body-content-typo') );
	
			if( $status_of_body_content_typo == '1' ) {
	
				$font = get_theme_mod('body-content-typo', consultancy_defaults('body-content-typo'));
			...
			$body_bg_color = get_theme_mod('body-bg-color', consultancy_defaults('body-bg-color'));
			$body_content_color = get_theme_mod('body-content-color', consultancy_defaults('body-content-color'));
			$body_a_color = get_theme_mod('body-a-color', consultancy_defaults('body-a-color'));
			$body_a_hover_color = get_theme_mod('body-a-hover-color', consultancy_defaults('body-a-hover-color'));
			...
			for ($i = 1; $i <= 6; $i++) :
				$status_of_h_typo = get_theme_mod( 'customize-body-h'.$i.'-typo', consultancy_defaults('customize-body-h'.$i.'-typo') );
				if($status_of_h_typo == 1){				
					$font = get_theme_mod('h'.$i.'',consultancy_defaults('h'.$i.''));
					$font_family    = isset($font['font-family']) ? $font['font-family'] : '';
					$font_size    	= isset($font['font-size']) ? $font['font-size'] : '';
		...
		public function consultancy_backend_editor_styles() {
			...
			wp_enqueue_style( 'consultancy-gutenberg', get_theme_file_uri('/css/admin-gutenberg.css'), false, CONSULTANCY_THEME_VERSION, 'all' );
			wp_add_inline_style( 'consultancy-gutenberg', $styles  );
