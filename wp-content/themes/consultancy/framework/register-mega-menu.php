<?php
add_filter( 'wp_edit_nav_menu_walker', 'consultancy_modify_backend_walker' , 10 );
function consultancy_modify_backend_walker() {
	return 'Consultancy_ModifyBackendMenuWalker';
}
/* ---------------------------------------------------------------------------
 * Admin Menu Walker
 * This is copy of wp-admin/includes/nav-menu.php
 * --------------------------------------------------------------------------- */
class Consultancy_ModifyBackendMenuWalker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'consultancy' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', 'consultancy'), $item->title );
		}
		...
		<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo esc_attr(implode(' ', $classes )); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo esc_html($submenu_text); ?>><?php esc_html_e( 'sub item', 'consultancy' ); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'consultancy'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'consultancy'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'consultancy'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'consultancy' ); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
				<?php if( 'custom' == $item->type ) : ?>
					<p class="field-url description description-wide">
						<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'URL', 'consultancy' ); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Navigation Label', 'consultancy' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Title Attribute', 'consultancy' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab', 'consultancy' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'CSS Classes (optional)', 'consultancy' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', 'consultancy' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Description', 'consultancy' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'consultancy'); ?></span>
					</label>
				</p>
                ...
                <p class="field-dt-use-as-megamenu description description-thin">
                    <label for="edit-menu-item-dt-use-as-megamenu-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Use As Mega Menu' ,  'consultancy' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-disable-text description description-thin">
                    <label for="edit-menu-item-dt-disable-text-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Disable Text' ,  'consultancy'); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-disable-link description description-thin">
                    <label for="edit-menu-item-dt-disable-link-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Disable Link' ,  'consultancy' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-fullwidth description description-thin">
                    <label for="edit-menu-item-dt-fullwidth<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Enable Full width' ,  'consultancy' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-submenu-column description description-wide">
                    ...
                    <?php esc_html_e( 'Sub Menu Column Layout' ,  'consultancy' ); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('Select where to align the submenu.',  'consultancy'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-menuicon description description-wide">
                    ...
                    <?php esc_html_e( 'Menu Icon' ,'consultancy'); ?><br/>
                    ...
                    <span class="description"><?php esc_html_e('Please use font awesome icon ',  'consultancy'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-menu-bg description description-wide">
                    ...
                    <?php esc_html_e( 'Menu Background' ,'consultancy'); ?><br/>
                    ...
                    <span class="description"><?php esc_html_e('Please use image url',  'consultancy'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-submenu-widget description description-wide">
                    ...
                    <?php esc_html_e( 'Display Widget Area' ,  'consultancy' ); ?><br />
                        <select id="edit-menu-item-dt-submenu-widget-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-dt-submenu-widget" name="dt-submenu-widget[<?php echo esc_attr($item_id); ?>]">
                            <option value=""><?php esc_html_e('Select', 'consultancy'); ?></option><?php
                            ...
                            foreach( $widgets as $widget) {
                                ...
                                $id = str_replace(" ", "-", $id);
                                echo "<option value='{$id}' " . selected ( $selected, $id, false ) . ">".$widget['widgetarea-custom-name']."</option>";
                            }?>
                        </select>
                    <span class="description"><?php esc_html_e('Select widget area to show.',  'consultancy'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-widgetarea-column description description-wide">
                        <label for="edit-menu-item-dt-submenu-column-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Fill Widget Area Layout' ,  'consultancy' ); ?><br />
                            <select id="edit-menu-item-dt-widgetarea-column-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-dt-submenu-column" name="dt-widgetarea-column[<?php echo esc_attr($item_id); ?>]">
                                <option value=""><?php esc_html_e('select','consultancy'); ?></option>
                            <?php for( $i = 2; $i <= 4; $i++): ?>
                              <option value="<?php echo esc_attr($i); ?>" <?php selected( $value,$i); ?>><?php echo esc_html( $i ); ?></option>
                            <?php endfor;?>
                            </select>
                        <span class="description"><?php esc_html_e('Only works if you choose widget area',  'consultancy'); ?></span>
                        </label>
                </p><!-- DesignThemes Custom Code Ends Here-->

				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php esc_html_e( 'Move', 'consultancy' ); ?></span>
						<a href="#" class="menus-move-up"><?php esc_html_e( 'Up one', 'consultancy' ); ?></a>
						<a href="#" class="menus-move-down"><?php esc_html_e( 'Down one', 'consultancy' ); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php esc_html_e( 'To the top', 'consultancy' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', 'consultancy'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e( 'Remove', 'consultancy' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'consultancy'); ?></a>
				</div>
 ...
 * To Save Menu Items
 * --------------------------------------------------------------------------- */
add_action( 'wp_update_nav_menu_item', 'consultancy_update_menu_item', 10, 3 );
function consultancy_update_menu_item( $menu_id, $menu_item_db, $args ) {

	$value = isset( $_POST['dt-use-as-megamenu'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-use-as-megamenu'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-use-as-megamenu',$value );

	$value = isset( $_POST['dt-disable-text'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-disable-text'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-disable-text',$value );

	$value = isset( $_POST['dt-disable-link'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-disable-link'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-disable-link',$value );

	$value = isset( $_POST['dt-fullwidth'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-fullwidth'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-fullwidth',$value );

	$value = isset( $_POST['dt-submenu-column'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-submenu-column'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-columns',$value );

	$value = isset( $_POST['dt-widgetarea-column'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-widgetarea-column'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-fill-columns',$value );

	$value = isset( $_POST['dt-menu-icon'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-menu-icon'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db,'_dt-menu-icon',$value );
	
	$value = isset( $_POST['dt-menu-bg'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-menu-bg'][$menu_item_db]) : "";
	update_post_meta( $menu_item_db, "_dt-menu-bg",$value);

	$value = isset( $_POST['dt-submenu-widget'][$menu_item_db] ) ? sanitize_text_field($_POST['dt-submenu-widget'][$menu_item_db]) : "";
 ...
 * --------------------------------------------------------------------------- */
class Consultancy_FrontEndMenuWalker extends Walker {
	...
	function start_lvl( &$output, $depth = 0, $args = array()) {
		...
		if( !($this->mega_active) ){
			$animation = get_theme_mod( 'menu-hover-style', consultancy_defaults('menu-hover-style') );
			if( !empty( $animation ) ) {
		...
		if( $this->dt_menubg ) {
			$output .= "\n$indent<ul class=\"sub-menu {$animation}\" style=\"background-image: url('{$this->dt_menubg}');\">\n";
		} else {
	...
	function end_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$indent = str_repeat("\t", $depth);
		$output .= "$indent</ul>\n";
		$output .= '<a class="dt-menu-expand" href="javascript:void(0)">+</a>';		
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$this->dt_menuimg = get_post_meta( $item->ID, '_dt-menu-bg', true);
		...
		if( empty($nolink) && empty($notext) ){
			...
			if( !empty( $this->dt_menuimg) ){
				$item_output .= '<i class="menu-item-icon menu-item-image">'.'<img src="'.$this->dt_menuimg.'"/>'.'</i>';
			}
		...
		if($depth === 0 ) {
			if( $this->mega_active ) {				
				...
				$sub_menu_style = get_theme_mod( 'sub-menu-style', consultancy_defaults('sub-menu-style') );
				if( !empty( $sub_menu_style ) ) {
					$class_names .= $sub_menu_style;
				}
