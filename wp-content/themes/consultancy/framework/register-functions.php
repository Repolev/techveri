 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('consultancy_features')) {

	function consultancy_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'consultancy-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'consultancy-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'consultancy-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'consultancy-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'consultancy-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'consultancy-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		...
		$skin = get_theme_mod('use-predefined-skin',consultancy_defaults( 'use-predefined-skin' ) );

		if( $skin ) {
			$skin            = get_theme_mod('predefined-skin',consultancy_defaults( 'predefined-skin' ) );
			$colors          = consultancy_skins( $skin );
			$primary_color   = $colors['primary-color'];
			$secondary_color = $colors['secondary-color'];
			$tertiary_color  = $colors['tertiary-color'];
			$quaternary_color = $colors['quaternary-default-color'];			
		} else {
			$primary_color   = get_theme_mod('primary-color',consultancy_defaults( 'primary-color' ) );
			$secondary_color = get_theme_mod('secondary-color',consultancy_defaults( 'secondary-color' ) );
			$tertiary_color  = get_theme_mod('tertiary-color',consultancy_defaults( 'tertiary-color' ) );
			$quaternary_color = get_theme_mod('quaternary-default-color', consultancy_defaults( 'quaternary-default-color' ) );
		}

		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'consultancy' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'consultancy' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'consultancy' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			),
			array(
				'name'  => esc_html__( 'Quaternary Color', 'consultancy' ),
				'slug'  => 'quaternary',
				'color' => $quaternary_color,
			),			
		));
	}
	add_action('after_setup_theme', 'consultancy_features');
}

/* ---------------------------------------------------------------------------
 * Check Activate Theme
 * --------------------------------------------------------------------------- */
if (!function_exists('consultancy_activation_function')) {
	function consultancy_activation_function( $old_name, $old_theme ) {
		...
		$options = array (
		  ...
		  'post-archives-readmore' => '[dt_sc_button title="Read More" icon_type="fontawesome" iconalign="icon-right with-icon" iconclass="fa fa-long-arrow-right" class="no-border-bg" /]',
		...
		if(!in_array($old_name, array('Consultancy', 'Consultancy Child'))) {
			update_option( '_cs_options', $options );
		}
	}
	add_action("after_switch_theme", "consultancy_activation_function", 10, 2 );
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('consultancy_under_construction') ){
	function consultancy_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'consultancy_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'consultancy_rand_css', 101);
	if( ! function_exists('consultancy_rand_css') ){
	function consultancy_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, CONSULTANCY_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_wp_list_categories') ){
	function consultancy_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'consultancy_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_wp_list_archive') ){
	function consultancy_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'consultancy_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_wpcf7_form_elements') ){
	function consultancy_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'consultancy_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Get Current Page or Page ID
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_ID') ){
function consultancy_ID(){
	global $post;
	$postID = false;
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_slide') ){
function consultancy_slider( $id = false ){
	...
	if( $id || is_home() || get_post_type() == 'page' ) {
		...
		elseif( is_page() ) $id = consultancy_ID();

		$slider_key = get_post_meta( $id, '_tpl_default_settings', true );

		$slider_key['slider_type'] =  isset($slider_key['slider_type']) ? $slider_key['slider_type'] : '';
		...
		if( is_array( $slider_key) && $slider_key['slider_type'] != '' ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( $slider_key['revolutionslider_id'] != '' ) && ( $slider_key['show_slider'] == 'true' ) ){
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= consultancy_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('consultancy_pagination') ){
function consultancy_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'consultancy') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'consultancy'),
					'next_text'          => esc_html__('Next', 'consultancy').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'consultancy_page_title' ) ){
	function consultancy_page_title( $echo = false ){
		...
		if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ){
			...
			$title = get_the_title( consultancy_ID() );
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'consultancy_breadcrumbs' ) ){
function consultancy_breadcrumbs(){
	global $post;
	
	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.get_theme_mod( 'breadcrumb-delimiter', consultancy_defaults('breadcrumb-delimiter') ).'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.get_theme_mod( 'breadcrumb-delimiter', consultancy_defaults('breadcrumb-delimiter') ).'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','consultancy'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  '<a href="'. $homeLink .'">'. esc_html__('Home','consultancy') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'consultancy').'</a>';

	} elseif( function_exists( 'is_shop' ) && is_shop() ){

		$breadcrumbs[] = '<span class="current">'. esc_html__('Shop', 'consultancy') .'</span>';

	} elseif( is_archive('dt_portfolios') ){

		$breadcrumbs[] = '<span class="current">'. esc_html__('Portfolios', 'consultancy') .'</span>';

	} elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(consultancy_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$cat = get_the_term_list(consultancy_ID(), 'product_cat', '', '$$$', '');
			$cats = array_filter(explode('$$$', $cat));
			if (!empty($cats))
				$breadcrumbs[] = $cats[0];
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( consultancy_ID() ) .'</span>';
		...
		foreach( $breadcrumbs as $bk => $bc ){
			if( !is_object( $bc ) ) {
				if( strpos( $bc , $separator ) ) {
					...
					echo apply_filters( 'consultancy_breadcrumbs', $bc );
				} else {
					if( $i == $count ) $separator = '';
					echo apply_filters( 'consultancy_breadcrumbs', $bc . $separator );
				}
			}
			$i++;
		}
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'consultancy_breadcrumb_section' ) ){
function consultancy_breadcrumb_section($id, $post_type ) {
	
	$bcrumb = (int) get_theme_mod( 'show-breadcrumb', consultancy_defaults('show-breadcrumb') );
	if( !empty( $bcrumb ) ) :
		...
		if( array_key_exists('breadcrumb_background', $settings) ):
			...
			if(!empty($bg) || !empty($co)) :
				...
				$bg = get_theme_mod( 'breadcrumb-bg-image' );
				$pos = get_theme_mod( 'breadcrumb-bg-position', consultancy_defaults('breadcrumb-bg-position') );
				$pos = !empty($position) ? $position : 'center center';
				$repeat = get_theme_mod( 'breadcrumb-bg-repeat', consultancy_defaults('breadcrumb-bg-repeat') );
				$repeat = !empty($repeat) ? $repeat : 'repeat';
				$co = get_theme_mod( 'breadcrumb-bg-color' );
				$co = !empty($co) ? $co : '';
				$attach = 'scroll';
				$size = 'auto';
			endif;
		...
		$bstyle = get_theme_mod( 'breadcrumb-style', consultancy_defaults('breadcrumb-style') );
...
if( ! function_exists( 'consultancy_breadcrumb_section_with_class' ) ){
function consultancy_breadcrumb_section_with_class ( $title , $class ){
...
if( ! function_exists( 'consultancy_events_title' ) ){
function consultancy_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'consultancy' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'consultancy'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'consultancy' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'consultancy' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'consultancy' );
		}
		...
		$title = esc_html__('Tag Archives','consultancy');
	}
	return $title;
}
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'consultancy_excerpt' ) ){
function consultancy_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'consultancy_wp_kses' ) ){
function consultancy_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_hex2rgb')) {

	function consultancy_hex2rgb($hex) {
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_header_logo')) {
function consultancy_header_logo() {
	echo '<div id="logo">';

	$use_logo = (int) get_theme_mod( 'use-custom-logo', consultancy_defaults('use-custom-logo') );

		if( class_exists( 'Kirki' ) ):
			$url 		= get_theme_mod( 'custom-logo', consultancy_defaults('custom-logo') );
			$darkbg_url = get_theme_mod( 'custom-dark-logo', consultancy_defaults('custom-dark-logo') );
			...
			<a href="<?php echo esc_url( home_url('/') ); ?>" rel="home">
				<img src="<?php echo CONSULTANCY_THEME_URI.'/images/logo.png'; ?>" alt="<?php echo esc_attr__('Logo', 'consultancy'); ?>" title="<?php echo esc_attr__('Logo', 'consultancy'); ?>"></a><?php
		endif;
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_comment_style')) {
function consultancy_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'consultancy');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'consultancy'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'consultancy');
					endif;
					edit_comment_link(esc_html__('Edit', 'consultancy'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'consultancy'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_get_page_permalink_by_its_template')) {
function consultancy_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_show_sidebar')) {
function consultancy_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('consultancy_global_variables')) {
function consultancy_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $consultancy_like_love;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'consultancy_like_love':
			return $consultancy_like_love;
		break;
	}
	return false;
}
