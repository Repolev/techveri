		...
		<div id="header-wrapper" class="<?php echo esc_attr( $class ); ?>">
            ...
            <header id="header"><?php
				...
				if( $htype != "left-header" && $htype != "left-header-boxed" && $htype != "creative-header" && $htype != "overlay-header" ):
					// header position
					$headerpos = get_theme_mod( 'header-position', consultancy_defaults('header-position') );
            		...
            		<div class="container">
            			...
            			<div class="main-header"><?php
							if( isset($htype) && ($htype == 'fullwidth-header header-align-center fullwidth-menu-header') ):
								$header_left = (int) get_theme_mod( 'enable-header-left-content', consultancy_defaults('enable-header-left-content') );
								if( !empty( $header_left ) ): ?>
									<div class="header-left"><?php									
									$leftcontent = get_theme_mod( 'header-left-content', consultancy_defaults('header-left-content') );
							...
							consultancy_header_logo();
                            
							if( isset($htype) && (($htype == 'fullwidth-header header-align-center fullwidth-menu-header') || 
								($htype == 'fullwidth-header header-align-left fullwidth-menu-header')) ):
								$header_right = (int) get_theme_mod( 'enable-header-right-content', consultancy_defaults('enable-header-right-content') );
								if( !empty( $header_right ) ):?>
									<div class="header-right"><?php
									if( class_exists('DTCorePlugin') ) {
										$rightcontent = get_theme_mod( 'header-right-content', consultancy_defaults('header-right-content') );
                            ...
                            if( isset($htype) && ( $htype == 'left-header' || $htype == 'left-header-boxed' || $htype == 'creative-header') ): ?>
                                <div class="left-header-footer"><?php
									$content = get_theme_mod( 'header-left-content', consultancy_defaults('header-left-content') );
									$content = do_shortcode( stripcslashes( $content ) );
									echo consultancy_wp_kses($content); ?></div><?php
							endif; ?>
            			</div>
            		</div>
            	</div><!-- **Main Header** --><?php
