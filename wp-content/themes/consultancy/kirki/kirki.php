...
class CONSULTANCY_Kirki {
	...
	public function __construct() {
		...
		add_action( 'enqueue_block_editor_assets', array( $this, 'enqueue_fonts' ), 10 );
		add_action( 'enqueue_block_editor_assets', array( $this, 'admin_enqueue_styles' ), 100 );
	}
	...
	public static function add_field( $config_id, $args ) {
		...
		self::admin_add_field($config_id, $args);
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $styles ) ) {
			...
			wp_enqueue_style( $current_theme->stylesheet . '-no-kirki', get_stylesheet_uri(), null, null );
			wp_add_inline_style( $current_theme->stylesheet . '-no-kirki', $styles );
		}
	}
	...
	public function get_styles() {
		...
		foreach ( $fields as $field ) {
			...
			foreach ( $field['output'] as $output ) {
				...
				if( ! array_key_exists( 'context', $output ) ) {
					$defaults = array(
						'element'       => '',
						'property'      => '',
						'media_query'   => 'global',
		...
		foreach ( self::$fields as $field ) {
			...
			if ( isset( $field['type'] ) && 'typography' == $field['type'] ) {
				...
				if ( isset( $value['font-family'] ) ) {
					...
					/** 
					 * To Load All variants
					 */
					$url .= ':100,100italic,200,200italic,300,300italic,regular,italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic';
	...
	public static function enqueue_fonts_url() {
 	...
 	public static function admin_add_field( $config_id, $args ) {
		// Kirki was not located, so we'll need to add our fields here.
		// check that the "settings" & "type" arguments have been defined
